...
import (
	...
	"cudos.org/cudos-node/testutil/network"
...
func networkWithNFTObjects(t *testing.T, n int) (*network.Network, []*types.NFT) {
	...
	for i := 0; i < n; i++ {
		state.NFTList = append(state.NFTList, &types.NFT{Creator: "ANY", Index: strconv.Itoa(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.NFT)
			}
		})
	}
}
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.NFT[j-i])
			}
		}
	})
